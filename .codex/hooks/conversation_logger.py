#!/usr/bin/env python3
"""Record simple user/assistant turns from Codex hook events."""
...
def state_dir(event: dict[str, Any]) -> Path:
    ...
    return Path(tempfile.gettempdir()) / "codex-conversation-logger" / safe_key(cwd)


def pending_path(event: dict[str, Any]) -> Path:
    return state_dir(event) / "pending" / f"{safe_key(event.get('session_id'), event.get('turn_id'))}.json"
